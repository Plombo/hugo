...
void build_gtk_interface (int argc, char* argv[])
{
  ...
  char ui_location[1024] = {"."};
  gboolean quit = FALSE;
	
  gtk_set_locale ();
  gtk_init (&argc, &argv);
  
  // open GtkBuilder file
  strncat(ui_location, "/hugoplus.ui", 12);
  gui_builder = gtk_builder_new();
  gtk_builder_add_from_file(gui_builder, "hugoplus.ui", NULL);
  ...
  */
  ...
  gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(fileselector_window), initial_path);

  main_window = create_mainWindow ();
  gtk_widget_show (main_window);
  sdl_socket = create_sdlSocket();
