...
// conversion array from bero's reduced pce palette to x11 palette
...
SDL_Surface* SetVideoMode(int width, int height, int bitsperpixel, int flags)
{
	...
	else
	{
		...
		sprintf(SDL_WindowID, "SDL_WINDOWID=%u", (unsigned int)(GDK_WINDOW_XWINDOW(gtk_widget_get_window(sdl_socket))));
		...
		gtk_window_resize(GTK_WINDOW(main_window), req.width, req.height);
	}
...
// TODO remove
int BlitToScreen(SDL_Surface* surface)
{
	return SDL_Flip(surface);
}

void osd_gfx_clear_screen()
{
	if(video_surface)
	{
		memset(video_surface->pixels, 0,
			video_surface->w * video_surface->h * video_surface->format->BytesPerPixel);
...
int osd_gfx_init(void)
{
  struct generic_rect rect;
	...
	const int fake_io_screen_w = 256;
  const int fake_io_screen_h = 240;
  ...
  if ((physical_screen = SetVideoMode(option.want_fullscreen ? option.fullscreen_width : fake_io_screen_w * option.window_size,
  ...
  {
    printf("SetVideoMode failed at %s:%d - %s\n", __FILE__, __LINE__, SDL_GetError());
    return 0;
  }
...
int osd_gfx_init_normal_mode()
{
  ...
  {
    physical_screen = SetVideoMode(io.screen_w * option.window_size, io.screen_h * option.window_size,
...
SInt32 osd_gfx_set_hugo_mode(SInt32 mode,SInt32 width,SInt32 height)
{
  ...
  screen = SetVideoMode(256,240, 8, SDL_SWSURFACE);
  SetPalette();	
  return !screen;
...
int ToggleFullScreen(void)
{
  ...
  if (physical_screen->flags & SDL_FULLSCREEN)
  {
    if ((physical_screen = SetVideoMode(io.screen_w * option.window_size, io.screen_h * option.window_size,
                                            (host.video.hardware_scaling ? 0 : 8),
                                            (host.video.hardware_scaling ? SDL_HWSURFACE : SDL_SWSURFACE) |
    ...
    if ((physical_screen = SetVideoMode(option.fullscreen_width, option.fullscreen_height,
                                            (host.video.hardware_scaling ? 0 : 8),
                                            (host.video.hardware_scaling ? SDL_HWSURFACE : SDL_SWSURFACE) |
